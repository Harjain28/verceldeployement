...
function () {
  function BusinessNotesComponent(api, http, router, fb, route) {
    ...
    this.route.params.subscribe(function (params) {
      ...
      localStorage.setItem("businessadminid", _this.userId); // if (this.id) {
      //   let requestData = {};
  ...
  BusinessNotesComponent.prototype.ngOnInit = function () {
    ...
    this.seletedValue = this.localdata.priority >= 6 ? 5 : this.localdata.priority; //console.log(this.seletedValue);
  };
  ...
  BusinessNotesComponent.prototype.deleteNotesDetails = function () {
    ...
    this.http.post(this.API_URL + "deletenotes", requestData, {
      headers: headers
  ...
  BusinessNotesComponent.prototype.getnotesDetails = function () {
    ...
    this.http.get(this.API_URL + "admingetnotes?userId=" + this.userId, {
      headers: this.httpOptions.headers
    }).subscribe(function (res) {
      //console.log(res, 'getnotesDetails');
      if (_this.id) {
        _this.notesData = res.notesData; //console.log(_this.htmlContent);

        _this.htmlContent = _this.notesData.filter(function (item) {
          return item._id === _this.id;
        }).map(function (item) {
          return item.notesdescription;
        }).toString(); //console.log(_this.htmlContent);
      } else {
  ...
  BusinessNotesComponent.prototype.submitaddNotes = function () {
    ...
    if (!this.id) {
      this.http.post(this.API_URL + "addnotes", requestData, {
        headers: headers
      }).subscribe(function (res) {
        //console.log(res);
        if (res.status == true) {
      ...
      this.http.post(this.API_URL + "editnotes", requestData, {
        headers: headers
      }).subscribe(function (res) {
        if (res.status == true) {
          _this.htmlContent = null;
  ...
  BusinessNotesComponent = __decorate([core_1.Component({
